...
export default function InvestmentCostPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setRows([])
        setLoadError("投資コストマスタの取得に失敗しました。")
        return
      }
      const values = Array.isArray(payload?.values) ? payload.values : []
      setRows(
        (values as MasterValue[])
          .filter((v) => v.category === "投資コスト")
  ...
  return (
    <>
      <PageHeader
        title="投資コストマスタ"
        description="出店時の初期投資費目（設備・内装・工事費など）を管理します。試算時の初期値として使用されます。"
      />
        ...
        <div className="mx-auto max-w-5xl px-8 py-7">
          <div className="rounded-lg border border-border bg-card overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              <div>
                <p className="text-sm font-semibold text-foreground">投資コスト費目</p>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">
              {dialogMode === "add" ? "投資コスト費目 — 新規追加" : "投資コスト費目 — 編集"}
            </DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4 py-2">
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                placeholder="例: investment_interior"
                value={form.code}
            ...
            <div className="rounded-md border border-border/60 bg-muted/20 p-3">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  id="investment-royalty-rule-enabled"
                />
                <Label htmlFor="investment-royalty-rule-enabled" className="text-xs">ロイヤリティ有無で金額を切り替える</Label>
              </div>
